...
// Registrar los valores recibidos en el log del servidor
error_log("DEBUG: ID Traslado: " . $id_traslado . " | ID Chofer Actual (Sesión): " . $id_chofer_actual);

try {
  /** @var PDO $conn */
  // 1. Buscamos un chofer que NO sea el actual, que tenga evaluaciones aprobadas
  $stmt = $conn->prepare("SELECT c.id_chofer
    FROM choferes c
    WHERE c.id_usuario != ?
    AND EXISTS (
